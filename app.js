...
`;
  ...
  renderer.listitem = (text, task, checked) => {
    if (task) {
      ...
      const check = checked
        ? '<svg width="15" height="15" viewBox="0 0 24 24" fill="none"><rect x="2" y="2" width="20" height="20" rx="4" fill="var(--tris-accent, #2563EB)"/><path d="M7 13l3 3 7-7" stroke="var(--tris-text-inverse, #fff)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" fill="none"/></svg>'
        : '<svg width="15" height="15" viewBox="0 0 24 24" fill="none"><rect x="2.5" y="2.5" width="19" height="19" rx="3.5" stroke="var(--tris-text-tertiary, #94A3B8)" stroke-width="1.5" fill="none"/></svg>';
  ...
  /* ---- On-demand loading of Mermaid / MathJax ----
     These two libraries are large (mermaid ~3.2MB, MathJax ~1.2MB) and most
     documents never use diagrams or math. Instead of loading them on every
     page view, we fetch them lazily, only the first time the current
     document actually needs one — and only once (cached via the *Loading
     promise below), no matter how many times render() runs afterwards. */
  ...
  function ensureMermaidLoaded() {
    ...
    if (!mermaidLoadPromise) mermaidLoadPromise = loadScriptOnce('vendor/mermaid.min.js');
  ...
  function ensureMathJaxLoaded() {
    ...
    if (!mathJaxLoadPromise) mathJaxLoadPromise = loadScriptOnce('vendor/mathjax-tex-mml-chtml.js');
  ...
  function render() {
    ...
    if (mermaidBlocks.length) {
      ensureMermaidLoaded().then(() => {
        initMermaid();
        try { mermaid.run({ nodes: preview.querySelectorAll('.mermaid') }); } catch (e) { /* ignore render errors */ }
      }).catch(() => { /* offline / blocked: leave raw mermaid source visible */ });
    }
    // MathJax — only fetched the first time the document contains math syntax.
    if (hasMathSyntax(body)) {
  ...
  function renderTabbar() {
    ...
    tabs.forEach(t => {
      ...
      el.innerHTML = isCheat
        ? `<span class="name bold"><span class="tris">Tris</span><span class="dot">.</span><span class="md">Markdown</span></span>`
  ...
  function syncHljsTheme(isDark) {
	  document.getElementById('hljsTheme').href =
		`vendor/hljs-themes/${isDark ? 'github-dark' : 'github'}.min.css`;
	}
...
img{max-width:100%;border-radius:8px;}
  ...
  const tocSidebarToggleIcon = document.getElementById('tocSidebarToggleIcon');
  if (tocSidebarToggle) {
    tocSidebarToggle.addEventListener('click', () => {
      const collapsed = tocSidebar.classList.toggle('mdv-toc-collapsed');
      tocSidebarToggle.title = collapsed ? 'Show' : 'Hide';
      tocSidebarToggleIcon.innerHTML = collapsed
